...
if TYPE_CHECKING:
    ...
    from app.medbox.schemas import ConflictReport
...
def format_citations_for_prompt(citations: list[Citation]) -> str:
    ...
    lines: list[str] = []
    # 按药品分组
    by_drug: dict[str, list[Citation]] = {}
    for c in citations:
        by_drug.setdefault(c.drug_name, []).append(c)

    for drug_name, refs in by_drug.items():
...
# ═══════════════════════════════════════════════════════════════
# 冲突检测报告格式化（任务四：规则引擎结论注入 prompt）
# ═══════════════════════════════════════════════════════════════


def format_conflict_report_for_prompt(report: ConflictReport) -> str:
    """将确定性规则引擎的 ConflictReport 格式化为 prompt 上下文。

    铁律 #1：冲突结论由规则引擎产出，LLM 只负责翻译成大白话，不得改写结论。
    铁律 #4：unresolved_drugs 非空时必须明示「暂未收录、无法检测」，不得静默忽略。

    返回文本供 build_system_prompt(conflict_context=...) 注入冲突槽位；
    即使「无冲突」也返回说明性文本，让 LLM 能如实转达「未检测到冲突」。
    """
    lines: list[str] = []
    ...
    if report.triggered_rules:
        lines.append("规则引擎检测到以下冲突（确定性结论，必须原样传达，不得否定或改写）：")
    ...
    if not lines:
        return "规则引擎未检测到成分重复或已知冲突。请如实告知用户目前未检测到冲突，但仍提醒按说明书用量服用。"
...
def build_system_prompt(
    ...
    """构造完整的 system prompt：角色 + 规则 + RAG 上下文 +（可选）冲突检测结论。

    citations 为 None 或空列表时，提示中说明无原文可用。
    conflict_context 非 None 时（任务四：意图为冲突检测），追加规则引擎结论槽位，
...
def build_chat_messages(
    ...
    包含 system prompt（含 RAG 上下文、可选冲突检测结论）、可选的对话历史、
    当前用户问题。
    """
    system = build_system_prompt(citations, conflict_context=conflict_context)
